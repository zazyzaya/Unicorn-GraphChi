...
for model_num, input_train_file in enumerate(train_files):
	with open(os.path.join(train_dir_name, input_train_file), 'r') as f:
		...
		# @cluster_max contains a max for each cluster
		cluster_max = [-1] * best_num_clusters
		...
		for cluster_idx in range(best_num_clusters):
			...
			cluster_max[cluster_idx] = max_dist
			cluster_thresholds[cluster_idx] = max_dist + NUM_STDS * std_dist
		...
		new_model = Model(cluster_medoids, cluster_max, cluster_thresholds, cluster_members, evolution)
