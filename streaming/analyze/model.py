...
for model_num, input_train_file in enumerate(train_files):
	with open(os.path.join(train_dir_name, input_train_file), 'r') as f:
		...
		# @cluster_mean contains a mean for each cluster
		cluster_mean = [-1] * best_num_clusters
		...
		for cluster_idx in range(best_num_clusters):
			...
			cluster_mean[cluster_idx] = mean_dist
		...
		new_model = Model(cluster_medoids, cluster_mean, cluster_thresholds, cluster_members, evolution)
